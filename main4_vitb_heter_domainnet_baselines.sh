...
OUTBASE="results/main4_spectral_vitb_hetero/domainnet/baselines"
LOGBASE="results/main4_spectral_vitb_hetero/domainnet/baselines/logs"

mkdir -p "$OUTBASE" "$LOGBASE"

# CUDA_VISIBLE_DEVICES=1 nohup python3 main.py $COMMON_ARGS \
#     --aggregation flex \
#     --output_dir "$OUTBASE" \
#     > "$LOGBASE/flex_100c_75r_heter_r32_domainnet_vitb.log" 2>&1 &
# echo "flex PID: $!"
...
CUDA_VISIBLE_DEVICES=2 nohup python3 main.py $COMMON_ARGS \
    --aggregation florist \
    --output_dir "$OUTBASE" \
    > "$LOGBASE/florist_100c_75r_heter_r32_domainnet_vitb.log" 2>&1 &
echo "florist PID: $!"
...
CUDA_VISIBLE_DEVICES=3 nohup python3 main.py $COMMON_ARGS \
    --aggregation flora \
    --output_dir "$OUTBASE" \
    > "$LOGBASE/flora_100c_75r_heter_r32_domainnet_vitb.log" 2>&1 &
echo "flora PID: $!"

echo "[$(date '+%H:%M:%S')] Waiting 60s before launching EXP2 (staggered init)..."
sleep 300

CUDA_VISIBLE_DEVICES=4 nohup python3 main.py $COMMON_ARGS \
    --aggregation hetlora \
    --output_dir "$OUTBASE" \
    > "$LOGBASE/hetlora_100c_75r_heter_r32_domainnet_vitb.log" 2>&1 &
echo "hetlora PID: $!"
