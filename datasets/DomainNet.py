...
# torch.backends.cudnn.enabled = False      # disabled: kills GPU throughput
# torch.backends.cudnn.deterministic = True  # disabled: forces slow deterministic kernels
...
def get_domainnet_dataset(base_path,domain_name, batch_size,alpha=0.01,clients_for_eachdomain=5,num_workers=0,classnum = 100):
    dataset_path = path.join(base_path)
    ...
    test_num_workers = min(4, os.cpu_count() or 4)
    test_dloader = DataLoader(test_dataset, batch_size=256, num_workers=test_num_workers, pin_memory=True, shuffle=False) 
    # added persistent_workers=True ~ Tanishk
...
def partition(alphaa, n_netss, y_train,classes=345):
    ...
    alpha = alphaa
    K=classes
    while min_size < 10: # changed from 10 to 1 ~tanishk
