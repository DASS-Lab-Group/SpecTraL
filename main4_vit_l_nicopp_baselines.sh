...
OUTBASE="results/main4_spectral_vit_l_hetero/nicopp/baselines"
LOGBASE="results/main4_spectral_vit_l_hetero/nicopp/baselines/logs"
...
CUDA_VISIBLE_DEVICES=0 nohup python3 main.py $COMMON_ARGS \
    --aggregation flex \
    --output_dir "$OUTBASE" \
    > "$LOGBASE/flex_100c_75r_heter_r32_nicopp_vitl.log" 2>&1 &
echo "flex PID: $!"
...
CUDA_VISIBLE_DEVICES=1 nohup python3 main.py $COMMON_ARGS \
    --aggregation florist \
    --output_dir "$OUTBASE" \
    > "$LOGBASE/florist_100c_75r_heter_r32_nicopp_vitl.log" 2>&1 &
echo "florist PID: $!"
